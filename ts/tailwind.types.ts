...
export type NRStackSpacing = `space-${Extract<Direction, 'x' | 'y'>}-${SpaceSize}`;
...
export type MarginSpacing = `m${MPDirection}-${SpaceSize | 'auto'}${
  ...
  | ''}`;
export type PaddingSpacing = `p${MPDirection}-${SpaceSize} p${MPDirection}-${SpaceSize}`;
...
type Strength = '50' | '100' | '200' | '300' | '400' | '500' | '600' | '700' | '800' | '900';
...
export type Display = 'block' | 'inline-block' | 'flex' | 'inline-flex' | 'hidden';

export type FlexDirection = 'flex-col' | 'flex-row' | 'flex-row-reverse' | 'flex-col-reverse';
